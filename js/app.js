...
***************************************************************************** */
    ...
    let collectionElement, cattegoriesElements;
    function windowload() {
        collectionElement = document.querySelector(".collection");
        cattegoriesElements = document.querySelectorAll(".collection__column");
        if (collectionElement) loadProduts();
    }
    async function loadProduts(start = 0, limit = 3) {
        const productsjson = "files/json/collections.json";
        const responce = await fetch(productsjson, {
            metod: "GET"
        });
        if (responce.ok) {
    ...
    function setProducts(products, start = 0, limit = 3) {
        ...
        productsArray.forEach(((product, index) => {
            if (index >= start && index < limit + start) {
                ...
                collectionItems.insertAdjacentHTML("beforeend", productTemplate);
            }
        }));
    }
    ...
    function documentActions(e) {
        ...
        if (targetElement.closest(".footer-collection__button")) {
            const productsQantity = document.querySelectorAll(".collection__item").length;
            loadProduts(productsQantity);
            e.preventDefault();
        }
